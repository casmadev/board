...
export interface CasmaBoardProps extends ShapeCallbacks {
  shapes?: ShapesState;
...
/* ------------------------------------------------------------------ */
/* Shape lifecycle callbacks                                          */
/* ------------------------------------------------------------------ */

/** A mutation-callback return type: either a partial patch to merge into the
 *  proposed shape (the board commits the merged result in the same state
 *  update — no flash), or `undefined`/`void` to accept the proposal as-is.
 *
 *  Returning a partial lets a consumer "edit the diff" inline. The classic
 *  use case is clamping a drag inside a region: receive the proposed shape,
 *  return `{ x: clampedX, y: clampedY }`, and the board commits the clamped
 *  position in the *same* render — the user never sees the out-of-bounds
 *  intermediate frame. */
export type ShapeMutationReturn = Partial<Shape> | void;

/** Lifecycle callbacks fired by the board when shapes change state. All are
 *  optional — pass only the ones you care about. Each receives the shape
 *  *as it exists after* the event was applied (so e.g. `onShapeAdd` sees the
 *  freshly-added shape, `onShapePatch` sees the post-patch state, and
 *  `onShapeDragEnd` sees the final snapped position).
 *
 *  Mutation-relevant callbacks (`onShapeAdd`, `onShapePatch`, the three
 *  `onShapeDrag*`, the three `onShapeEdit*`) can additionally return a
 *  `Partial<Shape>` that overrides fields of the proposed shape. The
 *  override is merged into the same state update — no double-render, no
 *  visible flash. Returning `void`/`undefined` accepts the proposed shape
 *  unchanged. Read-only callbacks (`onShapeSelect`, `onShapeDeselect`,
 *  `onShapeRemove`) ignore returns.
 *
 *  Notes on the contract:
 *  - `onShapeRemove` fires with the shape data captured *before* deletion —
 *    by the time the callback runs the shape is no longer in the board's
 *    state, so this is the only chance to read it.
 *  - `onShapeDeselect` only fires when the selection moves to something
 *    else (or to nothing); it does NOT fire when the previously-selected
 *    shape is removed (use `onShapeRemove` for that case).
 *  - `onShapeEditCommit` and `onShapeEditCancel` distinguish whether the
 *    edit was saved or thrown away; the board itself doesn't know which —
 *    it relies on the kind's renderer calling `onCommitEdit` vs
 *    `onCancelEdit` from its `ShapeRenderProps`.
 *  - `onShapePatch` fires for every mutation regardless of source —
 *    including the position patches generated by dragging. Subscribe to
 *    the drag-specific callbacks if you only want to react to user drags.
 *  - When both a drag callback and `onShapePatch` are wired up, the drag
 *    callback runs *first* and its override is folded into the patch
 *    before `onShapePatch` sees it. */
export interface ShapeCallbacks {
  /** Fired right before a shape is committed to the board (toolbar click,
   *  drag-from-toolbar commit, double-click spawn, or programmatic
   *  `addShape`). Returning a `Partial<Shape>` overrides fields before the
   *  shape lands in state. */
  onShapeAdd?: (shape: Shape) => ShapeMutationReturn;
  /** Fired immediately before a shape is removed from the board state. The
   *  argument captures the final state of the shape just prior to deletion.
   *  Returns are ignored. */
  onShapeRemove?: (shape: Shape) => void;
  /** Fired during patching, with the proposed merged shape and the partial
   *  that was passed. Returning a `Partial<Shape>` further overrides fields;
   *  the merged result is committed in the *same* state update. */
  onShapePatch?: (shape: Shape, patch: Partial<Shape>) => ShapeMutationReturn;
  /** Fired when a shape becomes the active selection. Does not fire when the
   *  same shape is re-selected (no-op). Returns are ignored. */
  onShapeSelect?: (shape: Shape) => void;
  /** Fired when a shape loses the active selection — either because the user
   *  selected something else or because the selection was cleared. Returns
   *  are ignored. */
  onShapeDeselect?: (shape: Shape) => void;
  /** Fired when a shape enters edit mode. Override returns are merged before
   *  edit mode officially begins (useful for resizing a shape into an
   *  editing layout). */
  onShapeEditStart?: (shape: Shape) => ShapeMutationReturn;
  /** Fired when a shape exits edit mode with its changes kept. `shape`
   *  reflects the post-edit state (assuming the kind's renderer patches
   *  before calling `onCommitEdit`, as the built-in sticky does). Override
   *  returns are merged in the same update. */
  onShapeEditCommit?: (shape: Shape) => ShapeMutationReturn;
  /** Fired when a shape exits edit mode with its changes discarded. */
  onShapeEditCancel?: (shape: Shape) => ShapeMutationReturn;
  /** Fired the moment a drag gesture crosses the 4px threshold and becomes a
   *  real drag. Sub-threshold taps (which only select) do not fire this. */
  onShapeDragStart?: (shape: Shape) => ShapeMutationReturn;
  /** Fired on every pointer move during a drag, *before* the position patch
   *  is committed. `shape.x` / `shape.y` reflect the proposed snapped target;
   *  return `{ x: clampedX, y: clampedY }` (or any other fields) to override
   *  what gets committed. Fires at high frequency — keep the work cheap. */
  onShapeDragMove?: (shape: Shape) => ShapeMutationReturn;
  /** Fired when a drag gesture ends (pointerup or pointercancel). `shape`
   *  reflects the proposed final position; override returns are committed in
   *  the same update. Sub-threshold gestures do not fire this. */
  onShapeDragEnd?: (shape: Shape) => ShapeMutationReturn;
}
